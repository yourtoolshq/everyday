...
export const businessRouter = createTRPCRouter({
  ...
  create: publicProcedure.input(businessActivityInput).mutation(async ({ ctx, input }) => {
    const household = await requireHousehold(ctx.db); const year = await requireActiveYear(ctx.db, household.id); await requirePerson(ctx.db, household.id, input.personId);
  ...
  update: publicProcedure.input(businessActivityUpdateInput).mutation(async ({ ctx, input }) => {
    const household = await requireHousehold(ctx.db); const year = await requireActiveYear(ctx.db, household.id); await requirePerson(ctx.db, household.id, input.personId); const { id, ...values } = input;
  ...
  delete: publicProcedure.input(z.object({ id: z.number().int().positive() })).mutation(async ({ ctx, input }) => {
    const household = await requireHousehold(ctx.db); const year = await requireActiveYear(ctx.db, household.id); const [activity] = await ctx.db.select({ taxItemId: businessActivities.taxItemId }).from(businessActivities).where(and(eq(businessActivities.id, input.id), eq(businessActivities.taxYearId, year.id))); if (!activity) throw new TRPCError({ code: "NOT_FOUND", message: "Self-employment business not found." }); await ctx.db.delete(taxItems).where(eq(taxItems.id, activity.taxItemId)); return { success: true };
  }),
});
