...
async function validatedClaim(
  ...
  const [visit] = await ctx.db.select().from(visits).where(eq(visits.id, input.visitId));
...
export const visitsRouter = createTRPCRouter({
  ...
  detail: publicProcedure.input(idInput).query(async ({ ctx, input }) => {
    ...
    const [person, careItem, provider, organization, visitDocuments, visitClaims] =
      await Promise.all([
  ...
  eligibleBenefits: publicProcedure.input(idInput).query(async ({ ctx, input }) => {
    const [visit] = await ctx.db.select().from(visits).where(eq(visits.id, input.id));
    if (!visit) return [];
    ...
    return allBenefits
      ...
      .map((benefit) => {
        ...
        const usedCents = benefit.openingUsedCents + paidAmounts.reduce((sum, amount) => sum + amount, 0);
        const remainingCents = Math.max(benefit.annualLimitCents - usedCents, 0);
        return {
          id: benefit.id,
          name: benefit.name,
          coverageScope: benefit.coverageScope,
  ...
  createClaim: publicProcedure.input(claimFieldsSchema).mutation(async ({ ctx, input }) => {
  ...
  deleteClaim: publicProcedure.input(idInput).mutation(async ({ ctx, input }) => {
    const [claim] = await ctx.db.delete(claims).where(eq(claims.id, input.id)).returning();
    if (!claim) throw new TRPCError({ code: "NOT_FOUND", message: "Claim not found" });
    return claim;
  }),
});
