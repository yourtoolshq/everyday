...
export const paycheques = sqliteTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.grossPayCents} >= 0 and ${table.incomeTaxCents} >= 0 and ${table.cppCents} >= 0 and ${table.cpp2Cents} >= 0 and ${table.eiCents} >= 0 and ${table.wiCents} >= 0 and ${table.ltdCents} >= 0 and ${table.extendedHealthCents} >= 0 and ${table.travelMedicalCents} >= 0 and ${table.unionDuesCents} >= 0 and ${table.otherDeductionsCents} >= 0 and ${table.netPayCents} >= 0`,
    ),
  ],
);
