...
function EmailAddressFields({ email }: { email: ParsedEml }) {
  ...
  return (
    <div className="flex shrink-0 items-start justify-between gap-4 text-sm">
      ...
      {email.date ? (
        <time className="shrink-0 text-right text-muted-foreground" dateTime={email.date}>
  ...
  useEffect(() => {
    ...
    void fetch(`/api/documents/${documentId}/eml`)
      ...
      .catch((fetchError: unknown) => {
        if (!cancelled) {
          setError(
            fetchError instanceof Error ? fetchError.message : "Unable to load email preview.",
          );
        }
      })
      .finally(() => {
