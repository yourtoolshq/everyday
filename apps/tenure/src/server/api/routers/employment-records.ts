...
async function loadEmploymentRecordData(
  ...
  const [documentRows, compensationRows, exceptionRows] = await Promise.all([
    ...
    ctx.db
      ...
      .orderBy(
        asc(compensationChanges.effectiveDate),
        asc(compensationChanges.createdAt),
      ),
    ctx.db
      .select({
  ...
  const exceptions =
    buildRecordExceptionsByEmployment(exceptionRows)[employmentId] ?? {};
...
export const householdPaySummaryProcedure = publicProcedure.query(
  async ({ ctx }) => {
    const paycheckRows = await ctx.db
      .select({
        payDate: paychecks.payDate,
