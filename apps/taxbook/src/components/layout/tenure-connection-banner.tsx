...
export function TenureConnectionBanner() {
  ...
  useEffect(() => {
    if (!status.data?.connected || status.data.linkedEmploymentCount === 0)
      return;
  ...
  if (!status.data.connected) {
    return (
      <div className="bg-destructive/10 text-destructive flex flex-wrap items-center justify-between gap-3 border-b px-4 py-2 text-sm">
        <span>
  ...
  if (status.data.linkedEmploymentCount > 0 && status.data.lastSyncAt) {
    const minutesAgo = Math.max(
      0,
      Math.round(
        (Date.now() - new Date(status.data.lastSyncAt).getTime()) / 60_000,
      ),
    );
    return (
