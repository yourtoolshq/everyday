...
export const businessRouter = createTRPCRouter({
  ...
  records: publicProcedure
    .input(z.object({ businessActivityId: z.number().int().positive() }))
    .query(({ ctx, input }) =>
      listBusinessRecords(ctx.db, input.businessActivityId),
    ),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      return ctx.db.transaction(async (tx) => {
        const [item] = await tx
          ...
          .values({
            taxYearId: year.id,
            name: `Net business income (loss) — ${input.name}`,
            taxLineReference: "T2125 9946 → 13500",
            type: "income",
            ownerKind: "person",
        ...
        const [activity] = await tx
          ...
          .values({ taxYearId: year.id, taxItemId: item!.id, ...input })
          .returning();
        return activity!;
      });
    }),
  update: publicProcedure
    .input(businessActivityUpdateInput)
    .mutation(async ({ ctx, input }) => {
      const household = await requireHousehold(ctx.db);
      const year = await requireEditableActiveYear(ctx.db, household.id);
      await requirePerson(ctx.db, household.id, input.personId);
  ...
  delete: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [activity] = await ctx.db
        .select({ taxItemId: businessActivities.taxItemId })
        .from(businessActivities)
        .where(
          and(
            eq(businessActivities.id, input.id),
            eq(businessActivities.taxYearId, year.id),
          ),
        );
      if (!activity)
      ...
      await ctx.db.delete(taxItems).where(eq(taxItems.id, activity.taxItemId));
      return { success: true };
    }),
});
