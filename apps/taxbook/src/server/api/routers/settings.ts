...
export const settingsRouter = createTRPCRouter({
  ...
  renameHousehold: publicProcedure
  ...
  addPerson: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [row] = await ctx.db
        .select({ value: max(people.sortOrder) })
        .from(people)
  ...
  renamePerson: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const result = await ctx.db
        ...
        .set({ name: input.name })
        .where(
          and(eq(people.id, input.id), eq(people.householdId, household.id)),
        )
  ...
  deletePerson: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const household = await requireHousehold(ctx.db);
      const [householdSize] = await ctx.db
        .select({ value: count() })
        .from(people)
        .where(eq(people.householdId, household.id));
      if ((householdSize?.value ?? 0) <= 1) {
        throw new TRPCError({
          code: "CONFLICT",
          message: "A household must have at least one person.",
        });
      }
      const [[itemReferences], [recordReferences]] = await Promise.all([
        ctx.db
          .select({ value: count() })
          .from(taxItems)
          .where(eq(taxItems.personId, input.id)),
        ctx.db
          .select({ value: count() })
          .from(records)
          .where(eq(records.personId, input.id)),
      ]);
      if (
        (itemReferences?.value ?? 0) > 0 ||
        (recordReferences?.value ?? 0) > 0
      ) {
        throw new TRPCError({
          code: "CONFLICT",
          message:
            "This person is referenced by tax information and cannot be removed.",
        });
      }
      const result = await ctx.db
        .delete(people)
