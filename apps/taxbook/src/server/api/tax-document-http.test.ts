...
describe("Tax Document multipart input", () => {
  ...
  it("requires issuer and a custom name for Other", async () => {
    ...
    await expect(
      parseCreateTaxDocumentForm(missingIssuer),
    ).rejects.toBeDefined();
  ...
  it("rejects unsupported, oversized, and invalid update attachments", async () => {
    ...
    await expect(parseCreateTaxDocumentForm(unsupported)).rejects.toMatchObject(
      { code: "BAD_REQUEST" },
    );
    ...
    oversized.set(
      "attachment",
      new File(
        [new Uint8Array(MAX_TAX_DOCUMENT_ATTACHMENT_BYTES + 1)],
        "large.png",
        { type: "image/png" },
      ),
    );
    await expect(parseCreateTaxDocumentForm(oversized)).rejects.toMatchObject({
    ...
    await expect(
      parseUpdateTaxDocumentForm(invalidAction),
