...
export async function requireActiveBusiness(db: QueryDb, id: number) {
  ...
  const [activity] = await db
    ...
    .where(
      and(
        eq(businessActivities.id, id),
        eq(businessActivities.taxYearId, year.id),
      ),
    );
  if (!activity)
    throw new TRPCError({
      code: "NOT_FOUND",
      message: "Self-employment business not found.",
    });
...
export async function syncBusinessTaxItem(db: QueryDb, activityId: number) {
  ...
  const rows = await db
    .select({
      kind: businessRecords.kind,
      amountCents: businessRecords.amountCents,
    })
    .from(businessRecords)
    .where(eq(businessRecords.businessActivityId, activityId));
  const totals = rows.reduce(
    (value, row) =>
      row.kind === "revenue"
        ? { ...value, revenue: value.revenue + row.amountCents }
        : { ...value, expenses: value.expenses + row.amountCents },
    { revenue: 0, expenses: 0 },
  );
  await db
    .update(taxItems)
    .set({
      actualAmountCents: totals.revenue - totals.expenses,
      updatedAt: new Date(),
    })
    .where(eq(taxItems.id, activity.taxItemId));
...
export async function listBusinessActivities(db: Database) {
  ...
  const rows = await db
    .select({
      ...
      taxYearId: businessActivities.taxYearId,
      personId: businessActivities.personId,
      personName: people.name,
      taxItemId: businessActivities.taxItemId,
      name: businessActivities.name,
      createdAt: businessActivities.createdAt,
      updatedAt: businessActivities.updatedAt,
    })
    .from(businessActivities)
    .innerJoin(people, eq(businessActivities.personId, people.id))
    .where(eq(businessActivities.taxYearId, year.id))
    .orderBy(asc(people.sortOrder), asc(businessActivities.id));
  const allRecords = await db
    .select({
      businessActivityId: businessRecords.businessActivityId,
      kind: businessRecords.kind,
      amountCents: businessRecords.amountCents,
    })
    .from(businessRecords)
    .innerJoin(
      businessActivities,
      eq(businessRecords.businessActivityId, businessActivities.id),
    )
    .where(eq(businessActivities.taxYearId, year.id));
  const items = rows.map((row) => {
    const totals = allRecords
      .filter((record) => record.businessActivityId === row.id)
      .reduce(
        (value, record) =>
          record.kind === "revenue"
            ? { ...value, revenue: value.revenue + record.amountCents }
            : { ...value, expenses: value.expenses + record.amountCents },
        { revenue: 0, expenses: 0 },
      );
    return {
      ...row,
      totals: { ...totals, net: totals.revenue - totals.expenses },
    };
  });
...
export async function listBusinessRecords(
  ...
  const { year, activity } = await requireActiveBusiness(
    db,
    businessActivityId,
  );
  const items = await db
    .select({
      ...
      businessActivityId: businessRecords.businessActivityId,
      kind: businessRecords.kind,
      expenseCategory: businessRecords.expenseCategory,
      date: businessRecords.date,
    ...
    .leftJoin(
      businessRecordAttachments,
      eq(businessRecords.id, businessRecordAttachments.businessRecordId),
    )
    .where(eq(businessRecords.businessActivityId, businessActivityId))
    .orderBy(desc(businessRecords.date), desc(businessRecords.id));
...
function validateYear(date: string, year: number) {
  if (!date.startsWith(`${year}-`))
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: `The Record date must be in the ${year} tax year.`,
    });
}
...
async function requireActiveBusinessRecord(db: QueryDb, id: number) {
  ...
  const year = await requireActiveYear(db as Database, household.id);
  const [row] = await db
    .select({ record: businessRecords, activity: businessActivities })
    .from(businessRecords)
    .innerJoin(
      businessActivities,
      eq(businessRecords.businessActivityId, businessActivities.id),
  ...
  if (!row)
    throw new TRPCError({
      code: "NOT_FOUND",
      message: "Self-employment Record not found.",
    });
  return { year, ...row };
}
...
export async function getBusinessRecordAttachment(db: Database, id: number) {
  await requireActiveBusinessRecord(db, id);
