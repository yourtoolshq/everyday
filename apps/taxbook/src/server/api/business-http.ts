...
import { allowedAttachmentTypes, MAX_ATTACHMENT_BYTES } from "~/domain/record";
import {
  businessRecordInput,
...
const nullableText = (form: FormData, name: string) =>
  textValue(form, name).trim() || null;
const positiveInteger = (form: FormData, name: string) => {
...
const common = (form: FormData) => ({
  ...
  expenseCategory: nullableText(form, "expenseCategory"),
  date: textValue(form, "date"),
  description: textValue(form, "description"),
  amountCents: positiveInteger(form, "amountCents"),
  notes: nullableText(form, "notes"),
});
export async function parseCreateBusinessRecord(form: FormData) {
  return {
    input: businessRecordInput.parse({
      businessActivityId: positiveInteger(form, "businessActivityId"),
      ...common(form),
    }),
    attachment: await attachmentFromForm(form),
  };
}
export async function parseUpdateBusinessRecord(form: FormData) {
  const input = businessRecordUpdateInput.parse(common(form));
  const action = textValue(form, "attachmentAction") || "keep";
  if (action === "keep" || action === "remove")
    return { input, attachmentAction: { type: action } as AttachmentAction };
  if (action !== "replace")
    throw new TRPCError({
      code: "BAD_REQUEST",
...
export function businessErrorResponse(error: unknown) {
  ...
  console.error("Self-employment request failed", error);
  return Response.json(
