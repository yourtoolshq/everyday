...
import type { PersonEstimateInput } from "./tax-calculator";
import { calculateHouseholdEstimate, progressiveTax } from "./tax-calculator";
...
describe("2026 Manitoba tax calculator", () => {
  it("applies progressive federal and Manitoba brackets", () => {
    expect(progressiveTax(5_852_300, rules2026Manitoba.federal.brackets)).toBe(
      819_322,
    );
    expect(progressiveTax(5_852_400, rules2026Manitoba.federal.brackets)).toBe(
      819_343,
    );
    expect(progressiveTax(4_700_000, rules2026Manitoba.manitoba.brackets)).toBe(
      507_600,
    );
    expect(progressiveTax(4_700_100, rules2026Manitoba.manitoba.brackets)).toBe(
      507_613,
    );
  });
  ...
  it("uses the best-known amount while an item is open and actual when complete", () => {
    expect(
      projectedManualAmount({
        status: "in_progress",
        expectedAmountCents: 100,
        actualAmountCents: 125,
      }),
    ).toBe(125);
    expect(
      projectedManualAmount({
  ...
  it("chooses the better medical claimant and preserves tuition remainders", () => {
    ...
    const result = calculateHouseholdEstimate(
      [lowIncome, person(2, 8_000_000)],
      {
        claimantPersonId: 1,
        medicalExpensesCents: 400_000,
        eligibleRentCents: 600_000,
        eligibleRentMonths: 6,
        eligibleSchoolTaxCents: 0,
        homeownerAdvanceReceivedCents: 0,
        homeOwnershipDays: 0,
      },
    );
    ...
    expect(
      result.people[0]!.federalTuition.remainingCents,
  ...
  it("applies losses without CPP and charges CPP only in remaining employment room", () => {
    ...
    const lossResult = calculateHouseholdEstimate([loss, person(2, 0)], {
      claimantPersonId: 1,
      medicalExpensesCents: 0,
      eligibleRentCents: 0,
      eligibleRentMonths: 0,
    ...
    const partialResult = calculateHouseholdEstimate(
      [partialRoom, person(2, 0)],
      {
        claimantPersonId: 1,
        medicalExpensesCents: 0,
        eligibleRentCents: 0,
    ...
    const noRoomResult = calculateHouseholdEstimate([noRoom, person(2, 0)], {
      claimantPersonId: 1,
      medicalExpensesCents: 0,
      eligibleRentCents: 0,
      eligibleRentMonths: 0,
