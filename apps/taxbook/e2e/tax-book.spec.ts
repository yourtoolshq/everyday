...
test("sets up a household and tracks an item", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Set up your Tax Book" }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("link", { name: "Example employment income", exact: true }),
  ).toBeVisible();
  await expect(page.getByText("10100", { exact: true })).toBeVisible();
  await page
    .getByRole("link", { name: "Example employment income", exact: true })
    .click();
  ...
  const attachmentHref = await page
    .getByRole("link", { name: "fictional-receipt.pdf" })
    .getAttribute("href");
  ...
  expect(attachmentResponse.headers()["x-content-type-options"]).toBe(
    "nosniff",
  );
  ...
  await page
    .getByRole("button", { name: "Example supporting expense", exact: true })
    .click();
  ...
  await page
    .getByRole("button", { name: "Actions for Example supporting expense" })
    .click();
  ...
  const taxDocumentAttachmentHref = await page
    .getByRole("link", { name: "fictional-t4.pdf" })
    .getAttribute("href");
  const taxDocumentAttachmentResponse = await page.request.get(
    taxDocumentAttachmentHref!,
  );
  expect(taxDocumentAttachmentResponse.ok()).toBe(true);
  expect(taxDocumentAttachmentResponse.headers()["content-type"]).toBe(
    "application/pdf",
  );
  expect(
    taxDocumentAttachmentResponse.headers()["x-content-type-options"],
  ).toBe("nosniff");
  expect(
  ...
  expect(
    taxDocumentDownloadResponse.headers()["content-disposition"],
  ).toContain("attachment");

  await page.getByRole("link", { name: "Tax Documents", exact: true }).click();
  await expect(
    page.getByRole("heading", { name: "Tax Documents" }),
  ).toBeVisible();
  ...
  await expect(
    page.getByText("Tax Estimate currently supports the 2026 tax year only."),
  ).toBeVisible();
  ...
  await page
    .getByRole("button", { name: "Actions for Example employment income" })
    .click();
  await page.keyboard.press("End");
  await page.keyboard.press("Enter");
  await expect(
    page.getByRole("heading", { name: "Delete this tax item?" }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("heading", { name: "Paycheques", level: 2 }),
  ).toBeVisible();
  ...
  await expect(
    page.getByText("No paycheques match these filters"),
  ).toBeVisible();
  await page.getByLabel("Filter by person").click();
  ...
  await expect(
    page.getByRole("link", {
      name: "Employment income — Employer A",
      exact: true,
    }),
  ).toBeVisible();
  await expect(page.getByRole("cell", { name: "$2,100.00" })).toBeVisible();
  ...
  await page.getByRole("link", { name: "Tax Estimate", exact: true }).click();
  await expect(
    page.getByRole("heading", { name: "Tax Estimate", exact: true }),
  ).toBeVisible();
  await expect(page.getByText("Projected household result")).toBeVisible();
  await expect(page.getByRole("button", { name: "Projected" })).toHaveAttribute(
    "aria-pressed",
    "true",
  );
  ...
  await expect(
    page.getByText("2026 Manitoba planning estimate—not filing software."),
  ).toBeVisible();
  ...
  await expect(
    page.getByText("Estimate settings", { exact: true }),
  ).toHaveCount(0);

  await page
    .getByRole("link", { name: "Self-employment", exact: true })
    .click();
  ...
  await expect(
    page.getByText("Self-employment business created."),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("link", {
      name: "Net business income (loss) — Web services",
      exact: true,
    }),
  ).toBeVisible();
  ...
  await page.getByRole("link", { name: "Tax Estimate", exact: true }).click();
  await expect(
    page.getByText("Net self-employment income (loss)").first(),
  ).toBeVisible();
  await expect(
    page.getByText("Self-employment CPP payable").first(),
  ).toBeVisible();
  ...
  await expect(
    page.getByText("Employment and its paycheques deleted."),
  ).toBeVisible();
