...
async function hasStatementDocument(
  db: Database,
  accountId: string,
  periodKey: string,
) {
...
export const statementPeriodExceptionsRouter = createTRPCRouter({
  ...
  listAll: publicProcedure.query(async ({ ctx }) => {
    return ctx.db
      ...
      .from(statementPeriodExceptions)
      .orderBy(
        asc(statementPeriodExceptions.accountId),
        asc(statementPeriodExceptions.periodKey),
      );
  }),

  create: publicProcedure
    .input(periodKeyInput)
    .mutation(async ({ ctx, input }) => {
      const account = await getAccountForPeriodException(
        ctx.db,
        input.accountId,
      );
      const hasDocument = await hasStatementDocument(
        ctx.db,
        input.accountId,
        input.periodKey,
      );
      const validation = canMarkPeriodNotApplicable(
        account,
        input.periodKey,
        hasDocument,
      );
      ...
      await ctx.db
        ...
        .onConflictDoUpdate({
          target: [
            statementPeriodExceptions.accountId,
            statementPeriodExceptions.periodKey,
          ],
          set: { updatedAt: now() },
        });
  ...
  delete: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (deleted.length === 0) {
        throw new TRPCError({
          code: "NOT_FOUND",
          message: "Period exception not found.",
        });
      }
