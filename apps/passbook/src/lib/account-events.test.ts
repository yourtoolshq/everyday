...
describe("activity attachments", () => {
  it("classifies opening activity files by type", () => {
    ...
    expect(activityAttachmentDocumentType("opening", pdf)).toBe("opening_document");
    expect(activityAttachmentDocumentType("opening", eml)).toBe("financial_correspondence");
    expect(activityAttachmentDocumentType("phone_call", pdf)).toBe("financial_correspondence");
  });
});
