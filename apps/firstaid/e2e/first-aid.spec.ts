...
test("plans a healthcare year and manages care items", async ({
  page,
}, testInfo) => {
  ...
  if (await firstPersonHeading.isVisible()) {
    ...
    await expect(
      page.getByRole("heading", { name: "Create your healthcare year" }),
    ).toBeVisible();
    await page.getByLabel("Plan year").fill(String(year));
    await page.getByRole("button", { name: "Create care plan" }).click();
  } else if (
    await page
      .getByRole("heading", { name: "Create your healthcare year" })
      .isVisible()
  ) {
    await page.getByLabel("Plan year").fill(String(year));
    await page.getByRole("button", { name: "Create care plan" }).click();
  ...
  await expect(
    page.getByRole("heading", { name: `${year} care plan` }),
  ).toBeVisible();
  ...
  await itemDialog
    .getByLabel("What care should be considered?")
    .fill("Dental cleaning");
  ...
  await page
    .getByRole("option", { name: "Preventive care", exact: true })
    .click();
  ...
  await expect(
    page.getByRole("heading", { name: "Dental cleaning" }),
  ).toBeVisible();
  ...
  await itemDialog
    .getByLabel("What care should be considered?")
    .fill("Dental exam and cleaning");
  await itemDialog.getByRole("button", { name: "Save changes" }).click();
  await expect(
    page.getByRole("heading", { name: "Dental exam and cleaning" }),
  ).toBeVisible();
...
test("manages providers and derives care progress from visits", async ({
  page,
}, testInfo) => {
  const suffix =
  ...
  await expect(
    page.getByText(organizationName, { exact: true }).first(),
  ).toBeVisible();
  ...
  await page.getByRole("option", { name: providerName }).click();
  await expect(
    page.getByRole("heading", { name: "Massage therapy goal" }),
  ).toBeVisible();
  ...
  await dialog.getByRole("button", { name: "Record visit" }).click();
  await expect(
    page.getByRole("heading", { name: "Unplanned lab visit" }).first(),
  ).toBeVisible();
});
...
test("manages household members safely", async ({ page }) => {
  ...
  await expect(
    dialog.getByText("Second Person", { exact: true }),
  ).toBeVisible();
  ...
  await expect(
    dialog.getByText("Household Member", { exact: true }),
  ).toBeVisible();
  await dialog.getByLabel("Delete Household Member").click();
  await page
    .getByRole("alertdialog")
    .getByRole("button", { name: "Delete person" })
    .click();
  await expect(
    dialog.getByText("Household Member", { exact: true }),
  ).toHaveCount(0);
...
test("attaches, finds, edits, opens, and cascade-deletes visit documents", async ({
  page,
  request,
}, testInfo) => {
  const suffix =
  ...
  await alert
    .getByRole("button", { name: "Delete visit and documents" })
    .click();
...
test("renders the responsive private application shell", async ({
  page,
}, testInfo) => {
  await page.goto("/");
  ...
  await expect(
    page.getByRole("link", { name: "Care Providers" }),
  ).toBeVisible();
...
test("tracks benefits, visit costs, and claims", async ({ page }, testInfo) => {
  ...
  if (
    await page
      .getByRole("heading", { name: "Who are you planning care for?" })
      .isVisible()
  ) {
  ...
  await expect(
    page.getByRole("heading", { name: "Employer plan" }),
  ).toBeVisible();
  ...
  await dialog.getByRole("button", { name: "Add organization" }).click();
  await expect(
    page.getByText(clinicName, { exact: true }).first(),
  ).toBeVisible();
  ...
  await dialog.getByRole("button", { name: "Add to plan" }).click();
  await expect(
    page.getByRole("heading", { name: careItemTitle }),
  ).toBeVisible();
  ...
  await dialog
    .getByLabel("Appointment date and time")
    .fill(`${year}-06-01T12:00`);
  ...
  await expect(
    page.getByText("Paid by benefits").locator("..").getByText("$100.00"),
  ).toBeVisible();
  await expect(
    page.getByText("Out of pocket").locator("..").getByText("$10.00"),
  ).toBeVisible();
  ...
  await expect(
    page
      .getByText("$110.00 cost · $100.00 reimbursed · $10.00 out of pocket")
      .first(),
  ).toBeVisible();
});
