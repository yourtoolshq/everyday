...
export const planningRouter = createTRPCRouter({
  overview: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      const [allPeople, plans, assignmentCounts, personVisitCounts] =
        await Promise.all([
      ...
      return {
        ...
        items: items.map((item) => {
          const itemVisits = linkedVisits.filter(
            (visit) => visit.careItemId === item.id,
          );
  ...
  deletePerson: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const total =
        (assignmentCounts[0]?.total ?? 0) + (visitCounts[0]?.total ?? 0);
      if (total > 0) {
        throw new TRPCError({
          code: "CONFLICT",
          message:
            "Reassign or remove this person’s care items and visits before deleting them.",
        });
      }
      const [person] = await ctx.db
        .delete(people)
  ...
  createPlan: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        databaseConflict(
          error,
          `A care plan for ${input.year} already exists.`,
        );
      }
    }),
