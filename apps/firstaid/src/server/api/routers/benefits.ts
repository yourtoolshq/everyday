...
export const benefitsRouter = createTRPCRouter({
  overview: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const [allPeople, allPlans, allBenefits, allClaims, allVisits] =
        await Promise.all([
          ctx.db.select().from(people).orderBy(asc(people.createdAt)),
          ctx.db
            .select()
            .from(insurancePlans)
            .orderBy(desc(insurancePlans.year)),
          ctx.db.select().from(benefits).orderBy(asc(benefits.name)),
          ctx.db.select().from(claims),
          ctx.db
            .select({ id: visits.id, startsAt: visits.startsAt })
            .from(visits),
        ]);
      ...
      const plansForYear = allPlans.filter(
        (plan) => plan.year === selectedYear,
      );
      const planIds = new Set(plansForYear.map((plan) => plan.id));

      const benefitsForYear = allBenefits.filter((benefit) =>
        planIds.has(benefit.insurancePlanId),
      );
      const benefitIds = new Set(benefitsForYear.map((benefit) => benefit.id));

      const claimsForBenefits = allClaims.filter((claim) =>
        benefitIds.has(claim.benefitId),
      );
      ...
      const years = Array.from(new Set(allPlans.map((plan) => plan.year))).sort(
        (a, b) => b - a,
      );
        ...
        plans: plansForYear.map((plan) => ({
          ...
          benefits: benefitsForYear
            ...
            .map((benefit) => {
              ...
              const usedCents = benefitUsedCents(
  ...
  createPlan: publicProcedure
    .input(insurancePlanFieldsSchema)
      ...
      if (planBenefits.length > 0) {
        ...
        if ((claimCount?.total ?? 0) > 0) {
          throw new TRPCError({
            code: "CONFLICT",
            message:
              "This insurance plan has claims on its benefits and cannot be deleted.",
          });
        }
      }
  ...
  createBenefit: publicProcedure
    .input(benefitFieldsSchema)
    .mutation(async ({ ctx, input }) => {
  ...
  benefitDetail: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return {
        ...
        person,
        usedCents,
        pendingCents: benefitPendingCents(
          benefitClaims.map((row) => row.claim),
        ),
        remainingCents,
        overLimitCents,
        resetDate: `${plan.year + 1}-01-01`,
